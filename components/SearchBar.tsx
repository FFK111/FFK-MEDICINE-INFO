...
export const SearchBar: React.FC<SearchBarProps> = ({ onSearch, isLoading }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-lg p-2 mb-8 border border-slate-100/10">
      <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2">
        <div className="w-full flex-grow flex items-center bg-slate-800/50 rounded-lg">
          <input
            type="text"
            value={query}
            onChange={(e) => {
            ...
            className="w-full p-3 bg-transparent focus:outline-none text-slate-100 placeholder-slate-400"
          ...
          <button
            ...
            className="p-3 rounded-lg text-slate-400 hover:text-blue-400 transition-all duration-200 disabled:opacity-50 transform hover:scale-110 active:scale-95"
          >
            <CameraIcon />
          </button>
        </div>
